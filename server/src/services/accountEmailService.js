...
export const accountEmailService = {
  ...
  async sendReminderEmail(userEmail, userName, entries) {
    try {
      ...
      const importantEntries = entries.filter(
        (e) => e.daysUntilDue > 2 && e.daysUntilDue <= 5
      );
          ...
          <table style="width: 100%; border-collapse: collapse;">
            <thead>
              <tr>
                ...
                <th style="padding: 10px; border: 1px solid #ddd; background-color: #f8f9fa;">Reminder Frequency</th>
              </tr>
            </thead>
            <tbody>
              ${entriesList
      ...
      <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto; padding: 20px;">
            ...
            `
              ...
              <div style="background-color: #fff3cd; border: 1px solid #ffeaa7; border-radius: 8px; padding: 20px; margin-top: 20px;">
                ...
                <ul style="margin: 0; color: #856404;">
                  ...
                  <li>Ensure billing is completed for urgent items</li>
                  <li>Contact relevant parties for immediate action</li>
                  <li>Update entries as needed</li>
                </ul>
        ...
        <div style="text-align: center; padding: 20px; color: #6c757d; font-size: 14px;">
          ...
          <p>Reminders are sent based on your configured reminder frequency settings.</p>
        </div>
      </div>
    `;
  ...
  async sendAccountCreatedEmail(userEmail, userName, accountDetails) {
    try {
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background-color: #f8f9fa; padding: 15px; border-radius: 5px; margin: 20px 0;">
            ...
            <p><strong>Billing Date:</strong> ${
              accountDetails.billingDate
                ? new Date(accountDetails.billingDate).toLocaleDateString()
                : "Not Set"
            }</p>
            <p><strong>Due Date:</strong> ${
              accountDetails.dueDate
                ? new Date(accountDetails.dueDate).toLocaleDateString()
                : "Not Set"
            }</p>
          </div>
      ...
      console.log(
        `✅ Account creation email sent successfully to ${userEmail}`
      );
    } catch (error) {
      console.error(
        `❌ Error sending account creation email to ${userEmail}:`,
        error
      );
      throw error;
    }
