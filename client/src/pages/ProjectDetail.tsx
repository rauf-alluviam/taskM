...
const ProjectDetail: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-slate-900 dark:via-slate-950 dark:to-black">
        <div className="text-center">
          <LoadingSpinner size="lg" />
          <p className="mt-4 text-slate-600 dark:text-slate-400 animate-pulse">Loading project details...</p>
        </div>
      </div>
    );
  }
  ...
  if (!project) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-slate-900 dark:via-slate-950 dark:to-black flex items-center justify-center">
        <div className="text-center bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700">
          ...
          <h2 className="text-xl font-semibold text-slate-800 dark:text-slate-200 mb-2">Project not found</h2>
          ...
          <Link to="/projects" className="inline-flex items-center text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 font-medium transition-colors">
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-slate-900 dark:via-slate-950 dark:to-black">
        ...
        <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div className="flex items-center space-x-4 min-w-0">
              <Link 
                to="/projects" 
                className="group p-3 text-slate-600 hover:text-blue-600 hover:bg-white/70 rounded-xl flex-shrink-0 transition-all duration-300 ease-out hover:scale-110 hover:shadow-lg dark:text-slate-400 dark:hover:text-blue-400 dark:hover:bg-slate-700/70"
              >
                <ArrowLeft className="w-6 h-6 transform group-hover:-translate-x-1 transition-transform duration-300" />
              </Link>
              <div className="min-w-0 flex-1">
                <div className="flex items-center space-x-3 mb-2">
                  <h1 className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-slate-800 to-slate-600 bg-clip-text text-transparent dark:bg-none dark:text-slate-100">
                    {project.name}
                  </h1>
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-xs font-medium text-slate-500 dark:text-slate-400">{Math.round(completionPercentage)}% Complete</span>
                  </div>
                </div>
                <p className="text-slate-600 dark:text-slate-400 text-sm sm:text-base line-clamp-2 leading-relaxed">{project.description}</p>
              </div>
            </div>
              ...
              <button
                ...
                className={`group relative overflow-hidden px-4 py-2.5 rounded-xl font-medium transition-all duration-300 ease-out border-2 ${
                  isMembersPopoverOpen 
                    ...
                    : 'bg-white/80 backdrop-blur-sm text-slate-700 border-slate-200 hover:bg-gradient-to-r hover:from-blue-500 hover:to-indigo-600 hover:text-white hover:border-blue-500 hover:shadow-lg hover:shadow-blue-500/25 dark:bg-slate-700/80 dark:text-slate-200 dark:border-slate-600 dark:hover:text-white'
                } transform hover:scale-105 hover:-translate-y-0.5`}
              >
                <div className="flex items-center space-x-2">
                  <Users className="w-4 h-4" />
                  <span className="hidden sm:inline">Members</span>
                  <span className="text-xs bg-slate-800/20 dark:bg-white/20 px-2 py-0.5 rounded-full">
              ...
              >
                <div className="p-6 border-b border-slate-200/50 dark:border-slate-700/50">
                  <div className="flex justify-between items-center">
                    <div>
                      <h3 className="font-semibold text-slate-900 dark:text-slate-100 text-lg">Project Team</h3>
                      <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{project.members?.length || 0} members</p>
                    </div>
                    {canManageMembers && (
                      <button
                        onClick={() => {
                  ...
                  {project.members && project.members.length > 0 ? (
                    project.members
                      ...
                      .map((member, index) => (
                        <div 
                          key={member.user._id} 
                          className="group flex items-center justify-between p-3 hover:bg-gradient-to-r hover:from-slate-50 hover:to-blue-50 rounded-xl transition-all duration-300 transform hover:scale-[1.02] dark:hover:from-slate-700 dark:hover:to-slate-600"
                          style={{ animationDelay: `${index * 50}ms` }}
                        >
                          <div className="flex items-center space-x-3 min-w-0 flex-1">
                            <div className="relative">
                              <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-full flex items-center justify-center flex-shrink-0 ring-2 ring-white dark:ring-slate-800 shadow-lg">
                            ...
                            <div className="min-w-0 flex-1">
                              <p className="font-medium text-slate-800 dark:text-slate-200 text-sm truncate">{member.user.name}</p>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-4 sm:p-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                <div className="flex items-center gap-4">
                    ...
                    <div><span className="text-sm text-slate-500 dark:text-slate-400">Department</span><p className="font-bold text-lg text-slate-800 dark:text-slate-100 truncate">{project.department}</p></div>
                </div>
                <div className="flex items-center gap-4">
                    <div className="bg-slate-100 dark:bg-slate-700 p-3 rounded-lg"><Calendar className="w-6 h-6 text-slate-500 dark:text-slate-400" /></div>
                    <div><span className="text-sm text-slate-500 dark:text-slate-400">Created</span><p className="font-bold text-lg text-slate-800 dark:text-slate-100">{new Date(project.createdAt).toLocaleDateString()}</p></div>
                </div>
                <div className="flex items-center gap-4">
                    <div className="bg-slate-100 dark:bg-slate-700 p-3 rounded-lg"><CheckSquare className="w-6 h-6 text-slate-500 dark:text-slate-400" /></div>
                    <div><span className="text-sm text-slate-500 dark:text-slate-400">Tasks</span><p className="font-bold text-lg text-slate-800 dark:text-slate-100">{projectTasks.length}</p></div>
                </div>
                <div className="flex items-center gap-4">
                    <div className="bg-slate-100 dark:bg-slate-700 p-3 rounded-lg"><Users className="w-6 h-6 text-slate-500 dark:text-slate-400" /></div>
                    <div><span className="text-sm text-slate-500 dark:text-slate-400">Members</span><p className="font-bold text-lg text-slate-800 dark:text-slate-100">{project.members?.length || 0}</p></div>
                </div>
            </div>
        </div>
      ...
      <Modal isOpen={showAddMemberModal} onClose={() => { setShowAddMemberModal(false); setSelectedUserIds([]); resetMember(); }} title="Add Project Member">
        <form onSubmit={handleSubmitMember(onAddMember)} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Select Users</label>
            ...
            {selectedUserIds.length === 0 && <p className="mt-1 text-sm text-red-600 dark:text-red-400">Please select at least one user</p>}
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Role</label>
            <select {...registerMember('role', { required: 'Please select a role' })} className="input-field w-full dark:bg-slate-700 dark:border-slate-600 dark:text-white">
        ...
        {selectedMember && selectedMember.user && (
          <form onSubmit={handleSubmitRole(onUpdateRole)} className="space-y-4">
            <div className="flex items-center space-x-3 p-4 bg-slate-50 dark:bg-slate-700/50 rounded-lg">
              ...
              <div><p className="font-medium text-slate-900 dark:text-slate-100">{selectedMember.user.name}</p><p className="text-sm text-slate-500 dark:text-slate-400">{selectedMember.user.email}</p></div>
            </div>
            <div>
            ...
            <div className="flex justify-end space-x-3 pt-4 border-t border-slate-200 dark:border-slate-700">
              <button type="button" onClick={() => { setShowUpdateRoleModal(false); setSelectedMember(null); }} className="btn-outline dark:text-slate-300 dark:border-slate-600 dark:hover:bg-slate-700">Cancel</button>
